...
if(connect_to_elmer) {
  ...
  eco_usxrpt <- fread(file = eco_usfile, header=TRUE) %>%                                          # load national series
...
# Woods & Poole import ---------------------------------------------------
  ...
  for(series in c("20", "21")) {
    wp_xrpt[1:4] <- lapply(wp_files[[series]],fread_wp)
    wp_xrpt %<>% rbindlist(use.names = TRUE) %>% .[,lapply(.SD, sum, na.rm=TRUE), by=d_year] %>%   # Aggregate to regional level
                 .[,c(1:2,18,92)] %>% setkey("d_year") %>% setnames(c(2:4), paste0("wp_", regvars)) %>%
                 .[, wp_hhsize := wp_pop/wp_hhs] # add HH size column
  ...
  rm(wp_files, fread_wp, wp_usfile)
